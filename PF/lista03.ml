...
let input_t (y: 'a) (xs: 'a list) =
  let rec aux xs acc =
    match xs with
    | [] -> List.map List.rev (fst acc) (* przy liczeniu permutacji rev nie będzie potrzebny *)
    | h::t -> let xs = h::(snd acc)
              in aux t ((y::xs)::(List.map (fun ys -> h::ys) (fst acc)), xs)
  in aux xs ([[y]], [])

(* fold na podstawie ogonowej *)
(* Wyniki sa odwrocone ale to nas nie boli przy liczeniu permutacji*)
...
let ins_ord_a (comp: 'a -> 'a -> bool) (y: 'a) (xs: 'a list) =
  let (b, xs) = List.fold_right (fun x (b, xs) -> if b && comp x y then (false, x::(y::xs))
                                                  else (b, x::xs))
                                xs
                                (true, [])
...
let ins_ord_b (comp: 'a -> 'a -> bool) (y: 'a) (xs: 'a list) =
  let (b, xs) = List.fold_left (fun (b, xs) x  -> if b && comp y x then (false, x::(y::xs))
                               else (b, x::xs))
                               (true, [])
                               xs
