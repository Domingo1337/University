...
void print_NFS3(int intervals, int size, double t[], double x[], double y[]) {
    double *Mx = moments_of(size, t, x);
    double *My = moments_of(size, t, y);
    double delta = 1. / ((double) intervals);
    double T = 0.;

    for (int k = 1; T <= 1. && k < size; k++) {
        double hk = t[k] - t[k - 1];
            while (T <= t[k]) {
